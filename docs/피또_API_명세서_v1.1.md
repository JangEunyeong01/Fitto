...
| 레시피·직접 입력 재료 (7장) | 완료 (로컬) | ✅ |
| 운동 (8장) | 완료 (로컬) | ✅ 조회·추가·수정·삭제 |
| 루틴 (8장) | 완료 (로컬) | ✅ 목록·저장·수정·삭제·일괄 기록 |
...
| `401` | 토큰 갱신 후 같은 작업 재시도 |
| 그 외 오류 | 재시도 횟수 증가, 5회 넘으면 버림 |

마지막 항목은 고칠 수 없는 요청 하나가 뒤의 기록을 영영 막지 않게 하려는 규칙이다.

내려받기 범위는 식단·운동이 오늘·어제 2일, 걸음수·체중·생리 컨디션이 30일이다. 식단·운동은 날짜별 조회라 기간만큼 요청이 늘어난다.
