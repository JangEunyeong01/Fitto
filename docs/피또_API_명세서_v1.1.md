...
화면은 항상 기기 저장소를 보고 그린다. 서버 통신은 그 뒤에서 따로 돈다.
...
**올리기 전에 받지 않는다.** 아직 못 올린 기록이 있는데 서버 값을 덮어쓰면 그 기록이 사라진다. 내려받기는 대기열이 비어 있을 때만 진행한다.

**대기열에는 값이 아니라 "무엇이 바뀌었는지"만 담는다.** 보낼 때 기기 저장소에서 최신 값을 읽으므로, 같은 대상을 여러 번 고쳐도 마지막 상태 한 번만 올라간다.

실패 처리:

| 상황 | 동작 |
|------|------|
| 연결 없음 | 대기열 유지, 실패로 세지 않음 |
| `401` | 토큰 갱신 후 같은 작업 재시도. 갱신도 거절되면 "로그인이 풀렸어요" 안내 |
| 삭제가 `404` | 이미 지워진 것으로 보고 성공 처리(응답 전에 끊겨 다시 보낸 경우) |
| 그 외 오류 | 재시도 횟수 증가, 5회 넘으면 **실패 목록으로 옮겨 설정 화면에 표시** |

마지막 항목은 고칠 수 없는 요청 하나가 뒤의 기록을 영영 막지 않게 하려는 규칙이다. 예전에는 버렸는데, 기기에만 남은 기록은 폰을 바꿀 때 사라지므로 버리지 않고 보여준다.

내려받기 범위는 식단·운동이 오늘·어제 2일, 걸음수·체중·생리 컨디션이 30일, 레시피·루틴·직접 입력 재료는 전체다. 템플릿은 서버 목록과 기기 목록을 합치고, 기기에만 있는 항목은 지우지 않고 올린다(예전에 대기열에 이 종류가 없어서 서버에 못 올라간 것들). 식단·운동은 날짜별 조회라 기간만큼 요청이 늘어난다.
...
| 항목 | 이전 | 바꾼 내용 | 이유 |
|------|------|-----------|------|
| `PATCH /users/me`의 물 목표 해제 (5장) | `waterGoal: null` | `waterGoalCustom: false` | JSON에서 "안 보냄"과 "null"이 구분되지 않는다. null을 "지우기"로 쓰면 앱이 빠뜨린 필드가 데이터를 지운다 |
...
**요청 횟수 제한** — 세 곳에 건다. 전부 메모리 기준이라 서버를 여러 대로 늘리면 인스턴스마다 따로 세고, 재시작하면 초기화된다.

| 대상 | 기준 | 한도 |
|------|------|------|
| 로그인 실패 | 이메일 / IP | 5분 10회 / 10분 30회 |
| 가입 | IP | 1시간 5회 |
| 쓰기 요청(POST·PUT·PATCH·DELETE) | 사용자 | 1분 300회 |

읽기는 세지 않는다. 데이터가 늘지 않기 때문이다. 쓰기 한도는 오프라인에 쌓인 대기열이 한꺼번에 올라와도 닿지 않는 값으로 잡았다.

**CORS** — 허용 주소는 환경변수로 받고 `*`는 쓰지 않는다. 인증을 `Authorization` 헤더로만 하므로 쿠키를 허용하지 않는다(`allowCredentials` 미설정). 쿠키를 안 쓰면 CSRF 위험도 함께 사라지고, 그래서 CSRF 토큰도 두지 않는다.

운영에서는 `localhost`·`127.0.0.1`·`*`·평문 `http`가 들어오면 부팅을 거부한다. 환경변수 값은 코드 리뷰에 걸리지 않아서 배포 주소를 추가할 때 개발용 주소를 지우지 않는 실수가 남는다.

**권한 모델** — 사용자 ID를 경로나 파라미터로 받는 API는 하나도 없다. 요청을 보낸 사람은 토큰에서만 정해진다(`/users/me`).

자원은 두 가지로 키를 잡는다.

| 방식 | 대상 | 조회 기준 |
|------|------|-----------|
| ID | 식단·운동·레시피·루틴·직접 입력 재료 | `(id, userId)` 두 값을 함께 본다 |
| 날짜 | 물·걸음·체중·주기 | `(userId, date)` 조합이 키 |

ID로 찾는 자원은 `id`만으로 조회하지 않는다. 남의 `id`를 넣으면 `404 NOT_FOUND`다 — 남의 것이라고 알려주지 않는다. `403`을 주면 그 `id`가 존재한다는 사실이 새어 나간다.

**출처 참조** — 운동의 `routineId`, 식단의 `recipeId`는 그 기록이 어디서 나왔는지 적어두는 값이다. 본인 것이 아니면 **거절하지 않고 그 값만 비워서** 저장한다. 루틴을 지운 뒤에 밀려 있던 기록이 올라오는 흐름이 정상이라, 거절하면 사용자가 실제로 한 기록이 대기열에서 버려진다.
...
**예외 메시지를 그대로 내보내지 않는다.** `500`의 `message`는 어떤 경우에도 고정 문구다. 스택 트레이스·SQL·JDBC 접속 정보·클래스명이 응답에 들어가지 않는다. 원인은 서버 로그에만 남긴다.

`errors[].field`는 요청 본문의 경로(`meals[0].name`)이지 서버 내부 필드가 아니다. 앱이 입력칸을 찾는 데 쓰는 값이라 그만큼만 준다.
...
> 라벨이 "중립형"에서 "차분형"으로 바뀌었지만 코드는 `neutral` 그대로다. 코드값을 바꾸면 이미 저장된 사용자 설정과 서버 enum까지 마이그레이션해야 하는데, 얻는 건 이름의 어울림뿐이다. **화면 문구를 고쳐도 데이터가 안 깨지는 것**이 코드값을 쓰는 이유이므로 여기서도 그 원칙을 따른다.
...
| 429 | `TOO_MANY_REQUESTS` (같은 이메일 5분에 10회 실패, 또는 같은 IP 10분에 30회 실패) |

제한을 두 축으로 건다. 이메일 기준만 두면 계정마다 카운터가 새로 시작해서 "이메일 1000개 × 각 1회"가 아무 제한에도 안 걸린다.

계정이 없어도 비밀번호 비교를 수행한다. 응답을 같게 맞춰도 BCrypt는 일부러 느린 함수라, 비교를 건너뛰면 응답 시간만으로 가입 여부가 드러난다.
...
갱신할 때마다 새 refreshToken으로 바꾼다(로테이션). 이미 쓴 토큰이 다시 오면 탈취로 보고 **그 사용자의 살아있는 토큰을 전부 폐기**한다. 폐기는 401을 던지는 트랜잭션과 분리해서 처리한다 — 같은 트랜잭션이면 예외에 롤백이 걸려 폐기가 취소된다.

**토큰 저장 위치(앱)** — 네이티브는 OS 보안 저장소(iOS 키체인 / 안드로이드 Keystore), 웹은 브라우저 저장소. 앱 기록과 저장소를 분리해 로그아웃이 기록을 건드리지 않게 한다.
...
메일로 받은 6자리 코드로 새 비밀번호를 정한다. 링크 방식이 아닌 이유: 링크는 누르면 앱으로 돌아오게 하는 딥링크와 받아줄 웹 페이지가 따로 필요하다. 코드는 메일에서 보고 옮겨 치면 끝이다.

#### POST /auth/password/forgot — 코드 요청

```json
{ "email": "user@example.com" }
```

**Response 202** No Content — **가입 여부와 상관없이 항상 같다**

| 규칙 | 내용 |
|------|------|
| 가입 여부 | 가입 안 된 이메일이어도 202. 메일만 안 나간다 |
| 응답 시간 | 계정 찾기·코드 해싱·메일 발송을 전부 응답 **뒤에서** 한다. 가입된 이메일일 때만 이 일들이 응답 전에 일어나면 수백 ms 늦어지고, 그걸 재면 가입 여부가 드러난다 |
| 재요청 | 같은 계정은 1분에 한 번. 1분 안에 다시 요청해도 202지만 메일은 안 나간다 |
| 요청 제한 | IP당 1시간에 10회 → `429 TOO_MANY_REQUESTS` |

#### POST /auth/password/reset — 코드로 재설정

```json
{ "email": "user@example.com", "code": "123456", "newPassword": "fitto5678" }
```

**Response 200** — 로그인 응답과 같다(`user`, `accessToken`, `refreshToken`). 재설정하고 바로 로그인된다.

| 규칙 | 내용 |
|------|------|
| 코드 | 숫자 6자리, 10분 유효, 한 번 쓰면 끝. 새로 받으면 이전 코드는 무효 |
| 틀린 횟수 | 코드 하나당 5회. 넘으면 맞는 코드도 받지 않는다. 이메일·IP 기준으로 1시간에 10회까지 |
| 다른 기기 | refreshToken을 전부 폐기한다(비밀번호 변경과 같은 이유) |
| 이메일 인증 | 코드를 맞혔으면 메일함을 연 것이므로 `emailVerified`도 true가 된다 |
| 저장 | 코드는 원본이 아니라 BCrypt 해시로 저장한다. 경우의 수가 백만 개뿐이라 SHA-256이면 DB가 새는 순간 되돌릴 수 있다 |

| 오류 | 코드 |
|------|------|
| 없는 계정·틀린 코드·만료된 코드 | `400 CODE_INVALID` — 셋을 구분하지 않는다 |
| 새 비밀번호 규칙 위반 | `400 INVALID_INPUT` |
| 시도 초과 | `429 TOO_MANY_REQUESTS` |

메일 서비스를 붙이기 전(운영)에는 코드 요청이 202로 끝나지만 메일이 나가지 않는다. 발송 실패는 서버 로그에만 남는다 — 응답으로 알리면 그 차이로 가입 여부가 드러난다.
...
|------|------|
| 보내지 않은 필드 | 그대로 유지 |
| `null`을 보낸 필드 | **보내지 않은 것과 같게 본다** (유지). 아래 "지우기" 참고 |
| 재계산 | [2-6](#2-6-재계산-시점) 기준 |
| `goals.waterGoal`에 숫자 | 그 값 저장, `waterGoalCustom: true` |
| `goals.waterGoalCustom`에 `false` | 직접 설정 해제, 계산값으로 되돌림 |
| `goals.targetCalorie` | 보내면 `400 INVALID_INPUT` (읽기 전용) |
| `email`, `password` | 이 API로 바꿀 수 없음 (2차) |

**"지우기"는 `null`이 아니라 별도 필드로 받는다.**

JSON에서는 "필드를 안 보냄"과 "null을 보냄"이 서버에서 똑같이 보인다. 둘을 구분하려면 `Optional` 같은 감싸는 타입이 필요한데, 실제로 구현해보니 Jackson은 필드가 없어도 빈 Optional을 만들어서 구분이 되지 않았다.

이 상태에서 `null = 지우기`로 정하면, 앱이 실수로 빠뜨린 필드가 "지워라"로 해석돼 프로필이 날아간다. 그래서 `null`은 안전한 쪽(유지)으로 해석하고, 지우려는 의도는 `waterGoalCustom: false`처럼 **명시적인 필드**로 받는다.
...
```json
{ "currentPassword": "fitto1234", "newPassword": "fitto5678" }
```

새 비밀번호 규칙은 가입과 같다 (8~64자, 영문+숫자). 변경으로 더 약한 비밀번호를 넣을 수 있으면 규칙이 무의미하다.

**Response 200** — 새 토큰 한 쌍
...
| 규칙 | 내용 |
|------|------|
| 다른 기기 | 그 계정의 refreshToken을 **전부 폐기**한다. 비밀번호를 바꾸는 이유 중 하나가 "누가 내 계정을 쓰는 것 같다"인데, 남의 기기가 로그인된 채면 바꾼 의미가 없다 |
| 이 기기 | 응답의 새 토큰으로 계속 쓴다. 앱은 이 토큰을 저장해야 한다 — 저장하지 않으면 자기 자신까지 로그아웃된다 |
| 시도 제한 | 10분에 5회. 현재 비밀번호를 찍어보는 걸 막는다. 탈퇴도 같은 값 |
...
가입은 바로 되고, 인증은 그 뒤에 받는다. 가입 전에 인증을 요구하면 "인증 대기 중인 가입"을 따로 보관해야 하고 게스트 기록 이전 흐름까지 길어진다.
인증을 받는 이유는 **오타 난 이메일로 가입하면 비밀번호를 찾을 방법이 없어서**다.

#### POST /users/me/email/verification — 코드 보내기

**Response 204** No Content. 이미 인증된 계정이면 아무것도 보내지 않는다.

| 오류 | 코드 |
|------|------|
| 1분 안에 다시 요청 | `429 TOO_MANY_REQUESTS` "코드는 1분에 한 번 받을 수 있어요." |
| 메일 서비스 미연결(운영) | `503 MAIL_UNAVAILABLE` |

#### POST /users/me/email/verification/confirm — 코드 확인

```json
{ "code": "123456" }
```

**Response 200** — User 리소스(`emailVerified: true`)

코드 규칙(10분, 5회, 한 번만)은 4-5와 같다. 틀리면 `400 CODE_INVALID`.

---

### DELETE /users/me — 회원 탈퇴

계정과 모든 데이터를 지운다. 되돌릴 수 없다.
...
**지우는 범위** — `user_id`를 가진 모든 표. 식단·식사 메모·운동·루틴·레시피·직접 입력 재료·수분·걸음수·체중·생리 설정과 기록·refreshToken, 그리고 계정(질환·알레르기·선호 음식 포함).

`users` 한 줄만 지우면 안 된다. `user_id` 컬럼이 있는 표는 18개인데 외래키로 `users`에 묶인 건 6개뿐이라, 나머지 표의 기록은 주인이 사라진 채 남는다. 표가 늘었을 때 삭제 코드에 추가하는 걸 잊으면 조용히 같은 일이 생기므로, 테스트가 DB에서 `user_id` 컬럼을 가진 표 목록을 직접 읽어 전부 비었는지 확인한다.
...
**개별 항목 제약은 각 API와 같다.** 한 번에 넣는 통로라고 느슨하게 두면 그게 우회로가 된다.

배열 길이도 막는다. 한 요청이 서버를 얼마나 오래 붙잡을 수 있는지가 여기서 정해진다.

| 대상 | 상한 |
|------|------|
| 식단·식단 메모·운동 | 각 10000 |
| 물·걸음·체중·주기 기록 | 각 4000 |
| 레시피·루틴·직접 입력 재료 | 각 500 |
...
| 429 | `TOO_MANY_REQUESTS` | 요청 횟수 제한 (로그인·가입·쓰기 요청) |
