...
export interface RequestOptions {
   ...
   *
   * **같은 요청을 두 번 보내도 괜찮은 API에만 쓴다.** 시간 초과는 "실패했다"가 아니라
   * "응답을 못 받았다"는 뜻이라, 서버가 그 사이 처리를 마쳤을 수 있다.
   * 비밀번호 변경을 다시 보내면 이미 바뀐 비밀번호로 검사해 실패하고,
   * 가입을 다시 보내면 방금 만든 계정 때문에 409가 온다.
   */
  wakeRetries?: number;
}
...
/** 서버를 깨우는 데 걸리는 시간(실측 93.8초)을 감안한 제한. iOS가 60초에 끊으므로 그 전에 끊는다. */
const WAKE_TIMEOUT_MS = 55_000;

/**
 * 잠든 서버를 먼저 깨운다.
 *
 * 무료 요금제 서버는 15분 쉬면 잠들고 다시 깨는 데 1분 30초쯤 걸린다.
 * 그동안 온 요청은 응답을 못 받지만 **서버는 깨어난 뒤 그 요청을 처리한다.**
 * 그래서 로그인·가입 같은 진짜 요청을 재시도하면 두 번 처리될 수 있다.
 *
 * 대신 아무 일도 하지 않는 /health를 먼저 두드려서 서버가 깨어난 걸 확인한 뒤 본 요청을 보낸다.
 * 이 요청은 몇 번을 보내도 서버 상태가 달라지지 않는다.
 *
 * 깨우기에 실패해도 예외를 던지지 않는다 — 본 요청이 어차피 같은 오류를 낼 것이고,
 * 사용자에게 보여줄 문구는 그쪽에서 정한다.
 */
export async function wakeServer(): Promise<void> {
  try {
    await fetchWithTimeout(`${BASE_URL}/health`, { method: 'GET' }, WAKE_TIMEOUT_MS, 2);
  } catch {
    // 깨우지 못했다. 본 요청에서 판단한다.
  }
}
